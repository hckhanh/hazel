...
module.exports = class Cache {
  ...
  async cacheReleaseList(url) {
    ...
    let { body } = await retry(
      async () => {
        const response = await fetch(url, { headers })

        if (response.statusCode !== 200) {
          throw new Error(
            `Tried to cache RELEASES, but failed fetching ${url}, status ${response.statusCode}`
          )
        }
    ...
    for (let i = 0; i < matches.length; i += 1) {
      ...
      body = body.replace(matches[i], nuPKG)
    }
    return body
  }
  ...
  async refreshCache() {
    ...
    const data = await retry(
      async () => {
        ...
        if (response.statusCode !== 200) {
          throw new Error(
            `GitHub API responded with ${response.statusCode} for url ${url}`
          )
        }
